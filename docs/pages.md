...
See [Architecture](architecture.md#routing) for routing and sidebar rules,
[the data layer](data-layer.md) for page queries, and [Components](components.md)
for the shared displays.
...
`LoadFile.tsx` holds the file and script controls described in
[the title-bar layout](architecture.md#title-bar-and-sidebar). It holds the
...
Load Pleco file and Profile info each write their own `<Table>`, and the two are **not** to be
...
See [Imports and storage](imports-and-storage.md) for import, restoration,
and incoming-link behavior.
...
Card count, Learning distribution, and Incoming reviews are documented in
[Charts](charts.md), including their calculations and links to card lists.
