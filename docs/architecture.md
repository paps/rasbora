...
- **A colour that is a _surface_ follows the scheme; a colour that is _data_
  ...
  from Mantine's variables as the CSS principle in [AGENTS.md](../AGENTS.md#principles) requires, and they follow
...
The bundled dictionary is another app-wide provider; see
[Dictionary](dictionary.md) for its lookup and loading rules.
...
See [incoming-link validation](imports-and-storage.md#incoming-link-validation)
for the constraints checked before routes mount, and [Pages](pages.md) for
...
`Layout.tsx`'s `PAGES` list is the sidebar, and **no link is ever disabled** — a page with
