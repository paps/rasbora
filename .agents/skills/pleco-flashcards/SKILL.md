...
description: Analyze a Pleco flashcard export (.pqb) directly with the sqlite3 CLI — card lists (most difficult, risky, almost learned, learned, customized), review history, dataset size. Use whenever a .pqb file needs read-only inspection or querying.
metadata:
  type: reference
---
...
- **Multi-valued settings are comma-*terminated*, not comma-separated** (`"1,"`, `"100,200,"`).
...
- **`history` reads newest-first**, one digit per review, Pleco's six-point grade — not a
  ...
  | 2 | forgotten *(default wrong answer)* | incorrect |
  | 3 | almost remembered | incorrect |
  | 4 | barely remembered | correct |
  | 5 | remembered | correct |
  | 6 | remembered perfectly *(default right answer)* | correct |
...
- `defn` non-empty is the *only* meaning a Pleco export carries — real dictionary definitions
...
2. Find the *oldest* incorrect one (`1`/`2`/`3`) in that window — call its index `failure`.
...
*and* customized" = the customized query's `WHERE` plus `AND s.incorrect > 0 AND s.correct > 0`,
